...
#ifdef USE_UART4
UART_Type * ESP_UART = UART4;
#else
UART_Type * ESP_UART = UART3;
#endif
//////////////////////
// Common Responses //
//////////////////////
...
const char sESP_UART[] =         "+UART_CUR";    // UART configuration for current session. Not in flash.
...
{
	UART_WriteBlocking(ESP_UART, (uint8_t *)"AT", 2);
	UART_WriteBlocking(ESP_UART, (uint8_t *)cmd, strlen(cmd));

	if (type == ESP8266_CMD_QUERY)
		UART_WriteBlocking(ESP_UART, (uint8_t *)'?', 1);
	else if (type == ESP8266_CMD_SETUP)
	{
		UART_WriteBlocking(ESP_UART, (uint8_t *)"=", 1);
		UART_WriteBlocking(ESP_UART, (uint8_t *)params, strlen(params));
	}

	UART_WriteBlocking(ESP_UART, (uint8_t *)"\r\n", 2);
}
...
unsigned int readByteToBuffer()
{
	...
	if (blockingmode)
	{
		...
		UART_ReadBlocking(ESP_UART, &receiveBuff, 1);
	}
	else
	{
...
char setBaud(const char * baud)
{
	...
	sendATcmd(sESP_UART, ESP8266_CMD_SETUP, parameters);
...
int16_t connect(const char * ssid, const char * pwd)
{
	...
	UART_WriteBlocking(ESP_UART, (uint8_t *)"AT", 2);
	UART_WriteBlocking(ESP_UART, (uint8_t *)ESP_CONNECT_AP, sizeof(ESP_CONNECT_AP)-1);
	UART_WriteBlocking(ESP_UART, (uint8_t *) "=\"", 2);
	UART_WriteBlocking(ESP_UART, (uint8_t *) ssid, strlen(ssid));
	UART_WriteBlocking(ESP_UART, (uint8_t *) "\"", 1);
	if (pwd != NULL)
	{
		UART_WriteBlocking(ESP_UART, (uint8_t *) ",", 1);
		UART_WriteBlocking(ESP_UART, (uint8_t *) "\"", 1);
		UART_WriteBlocking(ESP_UART, (uint8_t *) pwd, strlen(pwd));
		UART_WriteBlocking(ESP_UART, (uint8_t *) "\"", 1);
	}
	UART_WriteBlocking(ESP_UART, (uint8_t *) "\r\n", 2);
...
int16_t tcpConnect(const char * destination, const char * port, char keepaliveEnabled)
{
	...
	UART_WriteBlocking(ESP_UART, (uint8_t *) "AT", 2);
	UART_WriteBlocking(ESP_UART, (uint8_t *) ESP_TCP_CONNECT, sizeof(ESP_TCP_CONNECT)-1);
	UART_WriteBlocking(ESP_UART, (uint8_t *) "=\"", 2);
	UART_WriteBlocking(ESP_UART, (uint8_t *) "TCP", 3);
	UART_WriteBlocking(ESP_UART, (uint8_t *) "\"", 1);
	UART_WriteBlocking(ESP_UART, (uint8_t *) ",", 1);
	UART_WriteBlocking(ESP_UART, (uint8_t *) "\"", 1);
	UART_WriteBlocking(ESP_UART, (uint8_t *) destination, strlen(destination));
	UART_WriteBlocking(ESP_UART, (uint8_t *) "\"", 1);
	UART_WriteBlocking(ESP_UART, (uint8_t *) ",", 1);
	UART_WriteBlocking(ESP_UART, (uint8_t *) port, strlen(port));
	if (keepaliveEnabled)
	{
		UART_WriteBlocking(ESP_UART, (uint8_t *) ",", 1);
		UART_WriteBlocking(ESP_UART, (uint8_t *) "3500", 4);
	}
...
int16_t tcpSend(const uint8_t *buf)
{
	...
	if (rsp != ESP8266_RSP_FAIL)
	{
		...
		UART_WriteBlocking(ESP_UART, (uint8_t *) buf, size);
		UART_WriteBlocking(ESP_UART, (uint8_t *) "+IPD", 4);
